    ...
    public class Seeder : ISeeder
    {
        ...
        private async Task AddProduct(string title, string image1, string image2, string image3, decimal price, string desc, string additionalInfo, string category, int rate, List<(string, string)> productDescs)
        {
            ...
            if (!await _productDbService.AnyProductsByTitle(title))
            {
                ...
                for (int i = 0; i < productDescs.Count; i++) {
                    _productDescDbService.Add(new ProductDesc() { ProductId = product.Id, Name = productDescs[i].Item1, About = productDescs[i].Item2 });
                }
            }
        ...
        public async Task AddProducts()
        {
            ...
            await AddProduct("Мобільний телефон Samsung Galaxy M32 6/128 GB Light Blue (SM-M325FLBGSEK)", "samsung_galaxy_m32.png", "189581909.jpg", "189581950.jpg", 8555, "Екран (6.4\", Super AMOLED, 2400x1080) / MediaTek Helio G80 (2.0 ГГц + 1.8 ГГц) / основна квадрокамера: 64 Мп + 8 Мп + 2 Мп + 2 Мп, фронтальна камера: 20 Мп / RAM 6 ГБ / 128 ГБ вбудованої пам'яті + microSD (до 1 ТБ) / 3G / LTE / GPS / підтримка 2 SIM-карток (Nano-SIM) / Android 11 / 5000 мА·год", string.Empty, ProductCategoryConstant.TELEPHONE, rand.Next(3, 6), (new[] { 
                ("Серія", "Galaxy M"),
                ("Стандарт зв'язку", "4G (LTE)"),
                ("Діагональ екрана", "6.4"),
                ("Роздільна здатність дисплея", "2400 x 1080"),
                ("Тип матриці", "Super AMOLED"),
                ("Частота оновлення екрана", "90 Гц"),
                ("Кількість SIM-карток", "2"),
                ("Розміри СІМ-картки", "Nano-SIM"),
                ("Оперативна пам'ять", "6 ГБ"),
                ("Вбудована пам'ять", "128 ГБ"),
                ("Формат підтримуваних карт пам'яті", "microSD"),
                ("Максимальний обсяг підтримуваної карти пам'яті", "1 ТБ"),
                ("Операційна система", "Android"),
                ("Фронтальна камера", "20 Мп"),
                ("Особливості фронтальної камери", "Одинарна"),
                ("Розміщення фронтальної камери", "Крапелька"),
                ("Запис відео фронтальної камери", "Full HD/1920х1080/стереозвук"),
                ("Додатково", "Апертура f/2.2"),
                ("Назва процесора", "MediaTek Helio G80"),
                ("Тип ядра", "Cortex-A75 (64bit) + Cortex-A55 (64bit)\r\n"),
                ("Відеоядро", "ARM Mali-G52 MC2"),
                ("Кількість ядер", "8"),
                ("Серія MediaTek", "Helio G Series"),
                ("Частота процесора", "2.0 ГГц + 1.8 ГГц"),
                ("Основна камера", "64 Мп + 8 Мп + 2 Мп + 2 Мп"),
                ("Особливості основної камери", "Автофокус, Спалах, Ультраширококутний об'єктив"),
                ("Кількість основних камер", "4"),
                ("Запис відео основної камери", "Full HD/1920х1080/стереозвук"),
                ("Додатково", "Основна камера 64 Мп (f/1.8), Ультраширококутна камера 8 Мп (f/2.2), Макрокамера 2 Мп (f/2.4), Depth камера 2 Мп (f/2.4)"),
                ("Ємність акумулятора", "5000 мА·год"),
                ("Матеріал корпуса", "Пластик"),
                ("Роз'єми", "3.5 mm Mini-Jack, USB Type-C"),
                ("Роз'єм зарядного пристрою", "Type-C"),
                ("Роз'єм для підключення до ПК", "Type-C"),
                ("Роз'єм для навушників", "3.5 мм"),
                ("GPS-навігація", "BDS, GPS, ГЛОНАСС"),
                ("A-GPS", "Є"),
                ("Вага, г", "180"),
                ("Ширина", "74 мм"),
                ("Висота", "159.3 мм"),
                ("Глибина", "8.4 мм"),
                ("Безпека", "Сканер відбитків пальців збоку"),
                ("Датчики", "Акселерометр, Геомагнітний датчик, Гіроскоп, Датчик наближення, Датчик освітлення"),
                ("Комплект постачання", "Гарантійний талон, Документація, Зарядний пристрій, Кабель для синхронізації, Скріпка для виймання СІМ-карти, Телефон"),
                ("Бездротові технології", "Bluetooth 5.0, NFC, Wi-Fi"),
                ("Форм-фактор", "Моноблок"),
                ("Особливості корпусу", "Безрамковий дисплей"),
                ("Колір", "Light Blue"),
                ("Країна-виробник товару", "Китай"),
                ("Країна реєстрації бренду", "Республіка Корея (Південна Корея)"),
                ("Гаряча лінія виробника", "Samsung 0-800-502-000 Цілодобово 24/7"),
                ("Гарантія", "12 місяців"),
                ("Додаткові гарантійні умови", "Samsung PickUp Сервіс - безплатне відправлення в АСЦ мобільної техніки 0 800 502 000")
            }).ToList());
            await AddProduct("Мобільний телефон Apple iPhone 14 128GB Starlight (MPUR3RX/A)", "284913536.jpg", "284913565.jpg", "284913588.jpg", 41499, "Екран (6.1\", OLED (Super Retina XDR), 2532x1170) / Apple A15 Bionic / подвійна основна камера: 12 Мп + 12 Мп, фронтальна камера: 12 Мп / 128 ГБ вбудованої пам'яті / 3G / LTE / 5G / GPS / підтримка 2 SIM-карток (eSIM) / iOS 16", string.Empty, ProductCategoryConstant.TELEPHONE, rand.Next(3, 6), (new[] { 
                ("", ""),
                ("", "")
            }).ToList());
            await AddProduct("Мобільний телефон Samsung Galaxy S21 FE 6/128GB Lavender (SM-G990BLVDSEK/SM-G990BLVFSEK)", "245951562.jpg", "245951583.jpg", "245951595.jpg", 24999, "Екран (6.4\", Dynamic AMOLED 2X, 2340x1080) / Qualcomm Snapdragon 888 (2.84 ГГц) / потрійна основна камера: 12 Мп + 12 Мп + 8 Мп, фронтальна 32 Мп / RAM 6 ГБ / 128 ГБ вбудованої пам'яті / 3G / LTE / 5G / GPS / підтримка 2 SIM-карток (Nano-SIM) / Android 11 / 4500 мА·год", string.Empty, ProductCategoryConstant.TELEPHONE, rand.Next(3, 6), (new[] {
                ("", ""),
                ("", "")
            }).ToList());
            await AddProduct("Мобільний телефон Motorola G32 6/128GB Grey (PAUU0013RS)", "282291084.jpg", "282291085.jpg", "282291087.jpg", 7699, "Экран (6.5\", LCD, 2400x1080) / Qualcomm Snapdragon 680 (2.4 ГГц) / основная тройная камера: 50 Мп + 8 Мп + 2 Мп, фронтальная камера: 16 Мп / RAM 6 ГБ / 128 ГБ встроенной памяти + microSD (до 1 ТБ) / 3G / LTE / GPS / поддержка 2х SIM-карт (Nano-SIM) / Android 12 / 5000 мА*ч", string.Empty, ProductCategoryConstant.TELEPHONE, rand.Next(3, 6), (new[] { ("", ""), ("", "") }).ToList());
            await AddProduct("Мобільний телефон Nokia G10 3/32 GB Blue (719901148421)", "175052344.jpg", "175052345.jpg", "175052346.jpg", 3999, "Екран (6.5\", IPS, 1600x720) / MediaTek Helio G25 (2.0 ГГц) / потрійна основна камера: 13 Мп + 2 Мп + 2 Мп, фронтальна 8 Мп / RAM 3 ГБ / 32 ГБ вбудованої пам'яті + microSD (до 512 ГБ) / 3G / LTE / GPS / підтримка 2 SIM-карток (Nano-SIM) / Android 11 / 5050 мА·год", string.Empty, ProductCategoryConstant.TELEPHONE, rand.Next(3, 6), (new[] { ("", ""), ("", "") }).ToList());
            await AddProduct("Мобільний телефон Apple iPhone 11 128GB (PRODUCT) Red (MHDK3)", "152740976.jpg", "37399230.jpg", "37399239.jpg", 25499, "Екран (6.1\", IPS (Liquid Retina HD), 1792x828)/Apple A13 Bionic/основна подвійна камера: 12 Мп + 12 Мп, фронтальна камера: 12 Мп/RAM 4 ГБ/128 ГБ вбудованої пам'яті/3G/LTE/GPS/ГЛОНАСС/Nano-SIM/iOS 13/3046 мА·год", string.Empty, ProductCategoryConstant.TELEPHONE, rand.Next(3, 6), (new[] { ("", ""), ("", "") }).ToList());
            await AddProduct("Мобільний телефон Samsung Galaxy A53 5G 6/128GB Black (SM-A536EZKDSEK)", "263968716.jpg", "263968718.jpg", "263968720.jpg", 17299, "Екран (6.5\", Super AMOLED, 2400x1080)/ Samsung Exynos 1280 (2 x 2.4 ГГц + 6 x 2.0 ГГц)/ основна квадрокамера: 64 Мп + 12 Мп + 5 Мп + 5 Мп, фронтальна 32 Мп/ RAM 6 ГБ/ 128 ГБ вбудованої пам'яті + microSD (до 1 ТБ)/ 3G/ LTE/ 5G/ GPS/ A-GPS/ ГЛОНАСС/ BDS/ підтримка 2х SIM-карт (Nano-SIM)/ Android 12/ 5000 мА*год", string.Empty, ProductCategoryConstant.TELEPHONE, rand.Next(3, 6), (new[] { ("", ""), ("", "") }).ToList());
            await AddProduct("Мобільний телефон Samsung Galaxy A13 4/128GB Black (SM-A135FZKKSEK)", "263916044.jpg", "263916045.jpg", "263916047.jpg", 7999, "Екран (6.6\", PLS, 2408x1080) / Samsung Exynos 850 (2.0 ГГц) / основна квадрокамера: 50 Мп + 5 Мп + 2 Мп + 2 Мп, фронтальна 8 Мп / RAM 4 ГБ / 128 ГБ вбудованої пам'яті + microSD (до 1 ТБ) / 3G / LTE / GPS / підтримка 2х SIM-карт (Nano-SIM) / Android 12 / 5000 мА*год", string.Empty, ProductCategoryConstant.TELEPHONE, rand.Next(3, 6), (new[] { ("", ""), ("", "") }).ToList());
            await AddProduct("Мобільний телефон Samsung Galaxy M13 4/64GB Deep Green (SM-M135FZGDSEK)", "277025943.jpg", "277025945.jpg", "277025948.jpg", 7299, "Экран (6.6\", PLS, 2408x1080) / Samsung Exynos 850 (2.0 ГГц) / тройная основная камера: 50 Мп + 5 Мп + 2 Мп, фронтальная камера: 8 Мп / RAM 4 ГБ / 64 ГБ встроенной памяти + microSD (до 1 ТБ) / 3G / LTE / GPS / поддержка 2х SIM-карт (Nano-SIM) / Android 12 / 5000 мА*ч", string.Empty, ProductCategoryConstant.TELEPHONE, rand.Next(3, 6), (new[] { ("", ""), ("", "") }).ToList());
            await AddProduct("Мобільний телефон Apple iPhone 13 128GB Pink (MLPH3HU/A)", "221214151.jpg", "221214154.jpg", "221214153.jpg", 35999, "Екран (6.1\", OLED (Super Retina XDR), 2532x1170) / Apple A15 Bionic / подвійна основна камера: 12 Мп + 12 Мп, фронтальна камера: 12 Мп / 128 ГБ вбудованої пам'яті / 3G / LTE / 5G / GPS / Nano-SIM, eSIM / iOS 15", string.Empty, ProductCategoryConstant.TELEPHONE, rand.Next(3, 6), (new[] { ("", ""), ("", "") }).ToList());
            await AddProduct("Мобільний телефон Xiaomi Redmi 10 2022 4/128GB Pebble White", "220464250.jpg", "220464251.jpg", "220464252.jpg", 7999, "Екран (6.5\", 2400x1080) / Mediatek Helio G88 (2.0 ГГц + 1.8 ГГц) / основна квадрокамера: 50 Мп + 8 Мп + 2 Мп + 2 Мп, фронтальна 8 Мп / RAM 4 ГБ / 128 ГБ вбудованої пам'яті + microSD (до 512 ГБ) / 3G / LTE / GPS / підтримка 2 SIM-карток (Nano-SIM) / Android 11 / 5000 мА·год", string.Empty, ProductCategoryConstant.TELEPHONE, rand.Next(3, 6), (new[] { ("", ""), ("", "") }).ToList());

            await AddProduct("Куртка Puma Ferrari Race Mt7 Ecolit Jkt 53582502 S Rosso Corsa (4064537821299)", "288922454.jpg", "288922462.jpg", "288922474.jpg", 6290, "Розмір\r\nS\r\nОсобливості моделі\r\nЗ капюшоном\r\nСтиль\r\nСпортивний\r\nКолір\r\nЧервоний\r\nМатеріал верху\r\nПоліестер\r\nДовжина\r\nКороткий\r\nСклад\r\n100% полиэстер\r\nКраїна-виробник товару\r\nВ'єтнам\r\nКраїна реєстрації бренду\r\nНімеччина", "M", ProductCategoryConstant.CLOTH, rand.Next(3, 6), (new[] { ("", ""), ("", "") }).ToList());
            await AddProduct("Світшот Guess M2YQ27-KBA10-JBLK S Jet Black A996 (7621826345323)", "283267218.jpg", "283267236.jpg", "283267252.jpg", 2790, "Сезон\r\nВесняний\r\nОсінній\r\nРозмір\r\nS\r\nСтиль\r\nПовсякденний (casual)\r\nКолір\r\nЧорний\r\nМатеріал\r\nБавовна\r\nПоліестер\r\nОсобливості крою\r\nВільні\r\nСклад\r\n74% хлопок, 26% полиэстер\r\nПринт\r\nНапис\r\nКраїна-виробник товару\r\nТуреччина\r\nКраїна реєстрації бренду\r\nСША", "L", ProductCategoryConstant.CLOTH, rand.Next(3, 6), (new[] { ("", ""), ("", "") }).ToList());
            await AddProduct("Джинси Prodigy 32158-2 40 Темно-сині (2000002473336)", "289449186.jpg", "289449202.jpg", "289449211.jpg", 1182, "Посадка\r\nВисока\r\nМоделі\r\nRegular Fit\r\nРозмір\r\n40\r\nОсобливості моделі\r\nЗ кишенями\r\nКолір\r\nТемно-синій\r\nОсобливості крою\r\nШирокі\r\nЗастібка\r\nЗмійка\r\nСклад\r\n94.5% хлопок, 4% полиэстер, 1.5% эластан\r\nКраїна-виробник товару\r\nТуреччина\r\nКраїна реєстрації бренду\r\nТуреччина", "S", ProductCategoryConstant.CLOTH, rand.Next(3, 6), (new[] { ("", ""), ("", "") }).ToList());
            await AddProduct("Куртка дитяча Minoti 11COAT 1 37364JNR 110-116 см Темно-синя (5059030938512)", "287682045.jpg", "287682053.jpg", "287682056.jpg", 1529, "Дитячий вік\r\n5 років\r\n6 років\r\nЗріст\r\n110 см\r\n116 см\r\nСезон\r\nВесняний\r\nДемісезонний\r\nЗимовий\r\nОсінній\r\nКолір\r\nСиній\r\nОсобливості моделі\r\nЗ капюшоном\r\nПринт\r\nОднотонний\r\nМатеріал\r\nНейлон\r\nСклад\r\n100% нейлон\r\nКраїна-виробник товару\r\nМ'янма\r\nКраїна реєстрації бренду\r\nВеликобританія", "M", ProductCategoryConstant.CLOTH, rand.Next(3, 6), (new[] { ("", ""), ("", "") }).ToList());
            ...
            await AddProduct("Свисток суддівський Champion 5 шт", "219110202.jpg", "", "", 150, "Тип\r\nСвистки\r\nМатеріал\r\nМетал\r\nКолір\r\nХром\r\nДодаткові характеристики\r\nНеіржавка сталь\r\nВага: 0.02 г\r\nКраїна-виробник товару\r\nКитай\r\nКраїна реєстрації бренду\r\nУкраїна\r\nГарантія\r\n12 місяців", string.Empty, ProductCategoryConstant.SPORT, rand.Next(3, 6), (new[] { ("", ""), ("", "") }).ToList());
